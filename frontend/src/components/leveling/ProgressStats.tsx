...
import * as problemStateActions from '../../../../../src/features/problems/actions/problemStateActions';
import {ProblemState, ProblemStateDTO} from "../../../../../src/shared/models/ProblemState";
import {getProblemsForUser} from "../../../../../src/features/problems/actions/problemActions";

export default function ProgressStats() {
  const { isAuthenticated, user } = useAuth();
  const [progress, setProgress] = useState<ProblemStateDTO[]>([]);
  ...
  const fetchProgress = async () => {
    try {
      if (!user) return;
      const data = await getProblemsForUser();
      setProgress(Object.values(data));
    } catch (error) {
